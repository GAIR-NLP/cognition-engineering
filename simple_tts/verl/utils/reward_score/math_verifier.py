...
def compute_score(solution_str, ground_truth, reward_type='default') -> float:      
    if reward_type=='default':
        ...
        try:
            if "\\boxed" not in solution_str or solution_str.count("\\boxed")>=2: return -1.0
            if has_repeated_patterns(solution_str): return -1.0
            return correctness_score_v2(solution_str, ground_truth)
        except TimeoutException:
            return -1.0
        except Exception as e:
            return -1.0
    elif reward_type=="v3.repeatpenalty":
        try:
            if has_repeated_patterns(solution_str): return -2.0
            return correctness_score_default(solution_str, ground_truth)
        except TimeoutException:
            return -1.0
        except Exception as e:
            return -1.0
    elif reward_type=="v4.repeatpenalty":
        try:
            if solution_str.count("\\boxed")>=2: return -2.0
            if has_repeated_patterns(solution_str): return -2.0
            return correctness_score_default(solution_str, ground_truth)
        except TimeoutException:
            return -1.0
